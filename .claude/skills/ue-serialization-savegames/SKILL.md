...
UMyGameSaveGame* SaveData = Cast<UMyGameSaveGame>(UGameplayStatics::CreateSaveGameObject(UMyGameSaveGame::StaticClass()));
SaveData->PlayerHealth = 75.f;
// Then pass SaveData to SaveGameToSlot / AsyncSaveGameToSlot below
...
// The native-delegate overload takes a ULocalPlayer, not an APlayerController —
// the APlayerController overload takes the DYNAMIC delegate (FOnLocalPlayerSaveGameLoaded).
ULocalPlayerSaveGame::AsyncLoadOrCreateSaveGameForLocalPlayer(
    UMyLocalPlayerSave::StaticClass(), PlayerController->GetLocalPlayer(), TEXT("PlayerSlot0"),
