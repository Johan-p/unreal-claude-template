...
// Both handlers must be declared UFUNCTION() in the header for AddDynamic to bind
// (the macro goes on the declaration, never on the .cpp definition).
...
// Declare as UFUNCTION(BlueprintCallable, Category = "Cinematics") in the header —
// the macro goes on the declaration, never on the .cpp definition.
...
// Declared UFUNCTION() in the header (required for the AddDynamic binding above)
...
// Declared UFUNCTION(NetMulticast, Reliable) in the header; the .cpp defines the
// generated _Implementation body (UFUNCTION never goes on a .cpp definition)
void AMyNetworkGameMode::Multicast_RestorePlayerInput_Implementation()
{
    APlayerController* PC = GetWorld()->GetFirstPlayerController();
    if (PC)
    {
