...
**Tick-heavy tasks in non-instanced abilities:** `NonInstanced` is deprecated since UE 5.5
(UE_DEPRECATED_FORGAME) — the CDO runs the ability and ticking tasks on a CDO produce shared
state issues. Use `InstancedPerActor` or `InstancedPerExecution`.
