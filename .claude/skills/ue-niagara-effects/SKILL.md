...
### Deprecated since UE 5.3 — will be removed; use the FName variants above

The `SetNiagaraVariable*(const FString&, ...)` setters on `UNiagaraComponent` all carry
`UE_DEPRECATED(5.3, "This method will be removed in a future release. Please update to use the FName variant")`.
Do not write new code against them — use the `SetVariable*(FName, ...)` table above.
...
**Custom Data Interfaces**: Subclass `UNiagaraDataInterface`, implement
`virtual void GetFunctionsInternal(TArray<FNiagaraFunctionSignature>& OutFunctions) const` inside
`#if WITH_EDITORONLY_DATA` to define available functions (callers use `GetFunctionSignatures`;
`GetFunctions()` is `UE_DEPRECATED(5.4)`), `GetVMExternalFunction()` to bind C++ implementations, and optionally
