...
- **MapKey/UnmapKey at runtime for rebinding**: These are editor/config-screen helpers. For runtime rebinding use the 5.8 user-settings flow: `UEnhancedInputLocalPlayerSubsystem::GetUserSettings()` → `UEnhancedInputUserSettings::MapPlayerKey(FMapPlayerKeyArgs)` / `UnMapPlayerKey`. Requires `bEnableUserSettings` in Enhanced Input developer settings, and `FModifyContextOptions::bNotifyUserSettings = true` when adding IMCs. Alternatively swap contexts.
...
### Tag-Based Input Modes (Enhanced Input, 5.8)

UE 5.8 adds a tag-based input-mode system on Enhanced Input itself — distinct from `APlayerController::SetInputMode()` above (same name, different system). The subsystem tracks a `FGameplayTagContainer` describing the current input mode, and mapping contexts can be filtered by it:

- `IEnhancedInputSubsystemInterface::SetInputMode(const FGameplayTagContainer&)` — replace the current mode tags
- `AddTagToInputMode` / `AppendTagsToInputMode` / `RemoveTagFromInputMode` — incremental changes
- `GetInputMode` — query current mode tags
- Per-IMC: `InputModeFilterOptions` / `InputModeQueryOverride` control whether (and how) the context is filtered against the mode tags
- Gated by `bEnableInputModeFiltering` in `UEnhancedInputDeveloperSettings`

`APlayerController::SetInputMode()` still governs game-vs-UI focus and cursor routing; the tag-based system governs which Enhanced Input mapping contexts are active. They are independent.
