...
struct FMoveToTargetTask : public FStateTreeTaskBase
{
    ...
    // Handles are for real UObject/USTRUCT types the schema allows;
    // UStateTreeComponentSchema::IsExternalItemAllowed permits AActor,
    // UActorComponent, and UWorldSubsystem subclasses.
    // There is NO FStateTreeActorContext type — the owning actor comes
    // from Context.GetOwner() (see below), not from an external data handle.
    TStateTreeExternalDataHandle<UCharacterMovementComponent,
        EStateTreeExternalDataRequirement::Required> MovementHandle;
    ...
    virtual EStateTreeRunStatus EnterState(FStateTreeExecutionContext& Context,
        const FStateTreeTransitionResult& Transition) const override
    {
        // The owner of a UStateTreeComponent execution context IS the actor
...
struct FNearestEnemyEvaluator : public FStateTreeEvaluatorBase
{
    ...
    // No external data handle is needed just to reach the owning actor —
    // Context.GetOwner() returns it. Reserve TStateTreeExternalDataHandle<>
    // for real types the schema allows (AActor / UActorComponent /
    // UWorldSubsystem subclasses), and link them in Link().
