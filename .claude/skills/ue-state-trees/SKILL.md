...
External data provides typed references to objects outside the tree without going through evaluators. The template parameter is a **real UObject/USTRUCT type**, not a wrapper "context" struct — under `UStateTreeComponentSchema`, `IsExternalItemAllowed` only permits `AActor`, `UActorComponent`, and `UWorldSubsystem` subclasses, so anything else is rejected at edit time and never resolved.

For the **owning actor**, don't use an external-data handle at all — take it from the execution context: `AActor* Owner = Cast<AActor>(Context.GetOwner());`

```cpp
// Declare handles in your task/condition/evaluator struct — concrete types only
TStateTreeExternalDataHandle<UCharacterMovementComponent, EStateTreeExternalDataRequirement::Required> MovementHandle;
TStateTreeExternalDataHandle<UMyWorldSubsystem, EStateTreeExternalDataRequirement::Optional> SubsystemHandle;
...
// Access at runtime
UCharacterMovementComponent& Movement = Context.GetExternalData(MovementHandle);  // Required — reference
UMyWorldSubsystem* Subsystem = Context.GetExternalDataPtr(SubsystemHandle);       // Optional — pointer
```
