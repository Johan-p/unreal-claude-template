...
void UMyCMC::PhysWallRun(float deltaTime, int32 Iterations)
{
    ...
    // NOTE: assumes default gravity — under custom gravity use -GetGravityDirection() instead of FVector::UpVector.
...
private:
    FMyNetworkMoveDataContainer MoveDataContainer;
};
...
**Important:** The container must be a **member** that outlives the CMC — never a function-local `static`, which every CMC instance in the process would share. `SetNetworkMoveDataContainer` stores a pointer, not a copy. The engine follows the same pattern: `UCharacterMovementComponent::DefaultNetworkMoveDataContainer` is a per-instance member.
