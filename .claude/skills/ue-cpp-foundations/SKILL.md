...
description: Use when writing Unreal Engine C++ code involving UPROPERTY, UFUNCTION, UCLASS, TArray, TMap, delegates, FString, garbage collection, or smart pointers. Also use when the user asks about "UE C++", USTRUCT, UENUM, FName, FText, TObjectPtr, TWeakObjectPtr, UObject lifetime, UE_LOG, UE subsystems, or which C++ standard and standard-library types (std::vector, std::string, std::unique_ptr, C++20 features) are appropriate in engine code. For module build configuration, see ue-module-build-system. For Actor/Component architecture, see ue-actor-component-architecture.
metadata:
  version: 1.0.0
---
...
## Standard C++ inside UE — what's available, what UE supersedes

UE 5.8 compiles as **C++20** (`CppStandardVersion.Default` and `EngineDefault` are both `Cpp20` under `BuildSettingsVersion.V4`/`Latest`; only legacy V1–V3 targets fall back to C++17). Concepts, ranges, `if constexpr`, structured bindings, and designated initializers are therefore all usable.

Availability is not permission, though. For anything the engine provides its own type for, **the UE type wins** — it carries reflection, serialization, GC awareness, and the engine's allocators, none of which the standard type has:

| Standard | Use in UE | Why |
|---|---|---|
| `std::vector` / `map` / `set` | `TArray` / `TMap` / `TSet` | Reflectable in `UPROPERTY`, engine allocators, editor-visible |
| `std::string` | `FString`, `FName`, `FText` | Localization, interning, `TCHAR` correctness |
| `std::unique_ptr` / `shared_ptr` **for UObjects** | `TObjectPtr` under `UPROPERTY` | UObjects are garbage-collected, not RAII-owned — see the GC section above; `TSharedPtr<UMyObject>` is a real leak/double-free bug |
| `std::unique_ptr` / `shared_ptr` for plain types | `TUniquePtr` / `TSharedPtr` | Consistent with engine APIs and delegate binding (`BindSP`) |
| `std::mutex`, `std::thread` | `FCriticalSection` / engine task APIs | See `ue-async-threading` |
| `std::optional` / `variant` | `TOptional` / `TVariant` | Reflection-friendly equivalents |

**Where plain modern C++ *is* the right answer:** the engine-light rules layer — deterministic game logic written so automation tests can drive it without an engine world. Standard algorithms, `constexpr`, and value semantics belong there; everywhere else (actors, components, widgets, anything reflected) stays on UE types. RAII likewise applies normally to plain types and not at all to UObjects.
