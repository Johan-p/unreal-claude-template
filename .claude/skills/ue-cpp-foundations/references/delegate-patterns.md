...
> **Safety rule:** Unbind before the bound object dies, but use the right call. For `AddRaw(this, ...)`, `RemoveAll(this)` in `EndPlay` or the destructor works — the binding records `this` as its bound object. For `AddLambda`, `RemoveAll(this)` does **nothing**: a lambda binding has no bound object to match against, so it survives and will still fire. Store the `FDelegateHandle` returned by `AddLambda` and call `Remove(Handle)` instead (see "Binding lambda to multicast without storing the handle" below).
...
```cpp
// BAD — invisible to reflection; Blueprint cannot bind to it at all
// (Blueprint binding requires the UPROPERTY to be BlueprintAssignable)
