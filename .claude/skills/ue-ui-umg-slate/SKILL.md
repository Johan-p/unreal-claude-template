...
// Push screen onto the stack — AddWidget creates AND activates the widget internally,
// so do NOT call ActivateWidget() on the result
...
| Z-order conflicts — multiple widgets at the same Z-order gives undefined draw order | Pass an explicit Z-order to `AddToViewport(ZOrder)` (higher draws on top) and reserve a range per UI layer: HUD 0-9, menus 10-19, popups 20+ |
| Invisible widgets still tick — hidden widgets with `NativeTick` keep consuming CPU | Early-out with `if (!IsVisible()) return;` in `NativeTick`, or set visibility to `Collapsed`/`Hidden` so it stops ticking |
