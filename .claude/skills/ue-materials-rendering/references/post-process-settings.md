...
| `AutoExposureMinBrightness` | float | -10.0 (EV100) | Minimum scene luminance exposure adapts to. Units depend on `r.DefaultFeature.AutoExposure.ExtendDefaultLuminanceRange`: at 1 (UE5 template default) values are EV100, default -10.0; at 0 values are raw luminance, default 0.03. Setting "EV100 0.03" gives a black image |
| `bOverride_AutoExposureMaxBrightness` | bool | false | |
| `AutoExposureMaxBrightness` | float | 20.0 (EV100) | Maximum scene luminance exposure adapts to. With ExtendDefaultLuminanceRange=1 (UE5 template default): EV100, default +20.0; with the cvar at 0: raw luminance, default 8.0 |
...
| `AutoExposureLowPercent` | float | 10.0 | Histogram low percentile for metering (0–100). UE5 default 10.0 (80.0 was the UE4 value) |
| `bOverride_AutoExposureHighPercent` | bool | false | |
| `AutoExposureHighPercent` | float | 90.0 | Histogram high percentile for metering (0–100). UE5 default 90.0 (98.3 was the UE4 value) |
...
| `LumenSceneDetail` | float | 1.0 | Surface cache resolution scale. Range .25–4. Higher = better quality, higher cost |
| `bOverride_LumenSceneLightingQuality` | bool | false | |
| `LumenSceneLightingQuality` | float | 1.0 | Lighting pass quality. Range .25–2 |
...
| `LumenReflectionQuality` | float | 1.0 | Reflection quality. Range .25–2 |
