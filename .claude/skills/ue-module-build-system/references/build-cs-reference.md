...
```csharp
// IWYU (Include What You Use) is the default: IWYUSupport.Full.
// Opt out per-module when migrating legacy code that relies on transitive includes:
IWYUSupport = IWYUSupport.KeepAsIs;   // or IWYUSupport.None
```

`IWYUSupport` defaults to `IWYUSupport.Full` — each file must include every header it directly uses. The old `bEnforceIWYU` flag is obsolete since UE 5.2; setting it to true is a no-op.
...
```csharp
// Require a minimum x64 CPU architecture (enables AVX instructions)
MinCpuArchX64 = MinimumCpuArchitectureX64.AVX;
```

(The old `bUseAVX` flag was obsoleted and removed in 5.4.)
...
CppStandard = CppStandardVersion.Cpp20;   // UE default with BuildSettingsVersion.V4/Latest
CppStandard = CppStandardVersion.Cpp17;   // only the default under legacy settings versions (V1–V3)
```
