...
public class MyModule : ModuleRules
{
    public MyModule(ReadOnlyTargetRules Target) : base(Target)
    {
        ...
        // IWYU is the default (IWYUSupport.Full) — no per-module opt-in needed
...
PCHUsage = PCHUsageMode.UseExplicitOrSharedPCHs;
// IWYU defaults to IWYUSupport.Full; opt out per-module with
// IWYUSupport = IWYUSupport.KeepAsIs (or None) when migrating legacy code
...
**Edge case -- launcher vs source builds**: Launcher (binary) installs only include pre-built engine module binaries. Adding a dependency on an engine module not in the pre-built set causes LNK1104. Source builds from GitHub expose all engine modules. Use `Target.bUsePrecompiled` / `UnrealBuildTool.IsEngineInstalled()` to conditionally include source-build-only dependencies (`Target.LinkType` carries no installed-vs-source information).
...
- **Relying on transitive includes under IWYU** — IWYU is the default (`IWYUSupport.Full`): each file must include every header it uses directly, even if another include would pull it in.
