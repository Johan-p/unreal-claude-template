...
### Points: Circle (`UEnvQueryGenerator_OnCircle`)

Generates points on a single circle around a context, ray-traced outward from the center.

| Property | Type | Description |
|----------|------|-------------|
| `CircleRadius` | Float | Circle radius (cm) |
| `PointOnCircleSpacingMethod` | Enum | `BySpaceBetween` or `ByNumberOfPoints` |
| `SpaceBetween` / `NumberOfPoints` | Float / Int | Point spacing or fixed count (per spacing method) |
| `CircleCenter` | Context | Center of circle (e.g., `EnvQueryContext_Querier`) |
| `CircleCenterZOffset` | Float | Vertical offset of the center |
| `ArcDirection` + `ArcAngle` | Direction + Float | Optional arc restriction (enable `bDefineArc`) |
| `TraceData` | Trace | Horizontal trace for nearest obstacle |

### Points: Donut (`UEnvQueryGenerator_Donut`)

Generates points on concentric rings (annulus) around a context between two radii.

| Property | Type | Description |
|----------|------|-------------|
| `InnerRadius` / `OuterRadius` | Float | Min/max distance from context (cm) |
| `NumberOfRings` | Int | Number of rings to generate |
| `PointOnRingSpacingMethod` | Enum | `BySpaceBetween` or `ByNumberOfPoints` |
| `SpaceBetweenPoints` / `PointsPerRing` | Float / Int | Point spacing or count per ring (per spacing method) |
| `Center` | Context | Center of donut (e.g., `EnvQueryContext_Querier`) |
| `ArcDirection` + `ArcAngle` | Direction + Float | Optional arc restriction (enable `bDefineArc`) |
| `bUseSpiralPattern` | Bool | Spiral ring rotation instead of wheel-spoke alignment |

Use for: **flanking positions** (donut around enemy), **retreat points** (donut around self, arc facing away from enemy).
