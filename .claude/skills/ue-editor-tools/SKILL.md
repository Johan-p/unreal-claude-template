...
`UEdMode` (a `UCLASS`, from `Tools/UEdMode.h`) provides specialized viewport interaction. It is auto-registered by class discovery — no manual register/unregister call. Only one mode is active per viewport at a time.
...
{
    // Assigned in the UObject constructor — this is what registers the mode.
...
{
    Super::Enter();
    // Register interactive tools here via RegisterTool(...).
}
...
{
    Toolkit = MakeShared<FMyEditorModeToolkit>();
}
```

If the mode needs the legacy Slate toolkit / viewport hooks (`Render`, `HandleClick`, `MouseMove`, gizmo widgets), derive from `UBaseLegacyWidgetEdMode` (`Tools/LegacyEdModeWidgetHelpers.h`) instead of `UEdMode` — it is a `UEdMode` that also implements `ILegacyEdModeWidgetInterface` / `ILegacyEdModeViewportInterface`.

Legacy — do not use in new code: `FEdMode` (`EdMode.h`) plus `FEditorModeRegistry::Get().RegisterMode<T>()` / `UnregisterMode()` is the pre-UE5 path, kept only for existing modes.

---

## Asset Definitions

`UAssetDefinitionDefault` (a `UCLASS`, from `AssetDefinitionDefault.h`, module `AssetDefinition`) controls Content Browser appearance and context menu for custom asset types. It is auto-discovered — no registration or unregistration code.
...
public:
    ...
    // Override only if the default (UAssetEditorSubsystem) opening behaviour isn't enough.
    virtual EAssetCommandResult OpenAssets(const FAssetOpenArgs& OpenArgs) const override;
};
```

Legacy — do not use in new code: `FAssetTypeActions_Base` + `IAssetTools::RegisterAssetTypeActions` / `UnregisterAssetTypeActions` was replaced by the Asset Definition system in UE 5.2 (see the conversion guide in `AssetDefinition.h`); it survives only behind a compatibility proxy.
