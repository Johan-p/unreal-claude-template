...
virtual APlayerController* Login(UPlayer* NewPlayer, ENetRole InRemoteRole,
    ...
    const FUniqueNetIdRepl& UniqueId, FString& ErrorMessage);
virtual void PostLogin(APlayerController* NewPlayer);   // first safe point for RPCs (DispatchPostLogin deprecated 5.6 — override the protected virtual OnPostLogin(AController*), the designated post-login work override point in GameModeBase.h, also exposed as a BlueprintImplementableEvent; or override PostLogin directly)
...
Note: UE 5.8 also ships the newer `UE::Online` OnlineServices plugin as Epic's forward path for new projects; the Online Subsystem (OSS) remains supported.
