...
{
    // 5.8: engine nodes flow UPCGBasePointData — Cast<UPCGPointData> returns nullptr on it.
    const UPCGBasePointData* InPts = Cast<UPCGBasePointData>(In.Data);
    if (!InPts) continue;

    // Read via the base-class value-range accessors:
    const TConstPCGValueRange<FTransform> Transforms = InPts->GetConstTransformValueRange();
    const TConstPCGValueRange<int32> Seeds = InPts->GetConstSeedValueRange();

    UPCGPointData* OutPts = NewObject<UPCGPointData>();
    for (int32 i = 0; i < InPts->GetNumPoints(); ++i)
    {
        FPCGPoint NewPt;
        NewPt.Transform = Transforms[i];
        NewPt.Seed = Seeds[i];
...
Note: `GetPoints()` exists only on the `UPCGPointData` leaf class — a `Cast<UPCGPointData>` + `GetPoints()` loop compiles fine but silently skips the base-point data that engine nodes emit in 5.8.
