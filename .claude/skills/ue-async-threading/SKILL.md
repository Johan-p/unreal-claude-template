...
`FThreadSafeCounter` and `FThreadSafeBool` are discouraged in engine source comments (they carry no `UE_DEPRECATED` macro, so existing code is not broken) -- prefer `std::atomic<int32>` and `std::atomic<bool>` in new code.
