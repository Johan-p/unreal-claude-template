...
|----------|--------|------------|---------|
| `FTransformFragment` | `Mass/EntityFragments.h` | `FTransform Transform` | Entity world transform (location, rotation, scale) |
...
| `FAgentRadiusFragment` | `Mass/EntityFragments.h` | `float Radius` | Agent collision/avoidance radius |
| `FMassMoveTargetFragment` | `MassNavigationFragments.h` | `FVector Center`, `FMassInt16Real DesiredSpeed`, action via `GetCurrentAction()` | Navigation move target with speed and action |
...
5.8: these types moved to the MassCore module (`Mass/EntityFragments.h`, `Mass/EntityElementTypes.h`, `Mass/EntityHandle.h`, `Mass/ExternalSubsystemTraits.h`) — add MassCore to Build.cs. The old MassEntity headers (e.g. `MassCommonFragments.h`) are UE_DEPRECATED_HEADER(5.8).
...
Used by navigation and crowd systems. Contains the desired move target position (`FVector Center`), speed, and a movement action indicating the current navigation intent (move, stand, animate). Two field gotchas: `DesiredSpeed` is `FMassInt16Real`, not float — `MoveTarget.DesiredSpeed = 300.f;` won't compile; use `FMassInt16Real(300.f)`. The action is `EMassMovementAction`, read via `GetCurrentAction()` and set via `CreateNewAction` — there is no public `Action` member.
