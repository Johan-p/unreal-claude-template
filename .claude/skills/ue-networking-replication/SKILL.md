...
sends it to simulated proxies. `UCharacterMovementComponent` supersedes this path for
the *autonomous proxy*: it writes compressed moves via `FSavedMove_Character` and
reconciles them server-side.

**Leave `bReplicateMovement = true` on Characters — do not call `SetReplicateMovement(false)`.**
`ACharacter` gates four separate systems on `IsReplicatingMovement()`: `ReplicatedBasedMovement`
replication, networked root motion, `ReplicatedMovementMode` + gravity-direction packing, and
simulated-proxy pose ticking. Disabling it breaks simulated proxies, based movement (standing on
moving platforms), and networked root motion — a bug that only shows up in a real multiplayer
session, never in PIE single-player.
