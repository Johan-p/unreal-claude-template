...
{
    // Store the id as a member — the async callback below needs it
...
void UExperienceManagerComponent::OnExperienceLoadComplete()
{
    ...
    CurrentExperience = Cast<UExperienceDefinition>(
        UAssetManager::Get().GetPrimaryAssetObject(CurrentExperienceId));
    ...
    {
        // Entries are plugin NAMES — LoadAndActivateGameFeaturePlugin needs a
        // plugin URL. Resolve the descriptor path and convert it (see
        // UGameFeaturesSubsystem::GetPluginURL_FileProtocol in SKILL.md).
        FString PluginURL;
        if (TSharedPtr<IPlugin> Plugin = IPluginManager::Get().FindPlugin(PluginName))
        {
            PluginURL = UGameFeaturesSubsystem::GetPluginURL_FileProtocol(
