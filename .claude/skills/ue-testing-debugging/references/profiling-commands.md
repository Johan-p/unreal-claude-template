...
```bash
# Capture 10 seconds of CPU/frame trace then exit
UnrealEditor-Cmd MyGame -game \
    -trace=cpu,frame \
    -tracefile=MyCapture.utrace \
    -ExecCmds="delay 10, quit" \
    -log
```

Open the resulting `.utrace` in `UnrealInsights.exe` — see **Unreal Insights** below.
...
| `-ExecCmds="Trace.Start cpu,frame"` | Begin an Insights trace on startup |
