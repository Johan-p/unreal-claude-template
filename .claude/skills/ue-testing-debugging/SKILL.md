...
// GOTCHA — float vs double overloads: UE5 vector components are `double`, and
// AutomationTest.h declares SEPARATE float and double overloads of TestEqual.
// Compare whole vectors with the FVector overload; when comparing a single
// component, use a double literal or the overloads are ambiguous (C2666):
TestEqual(TEXT("Whole vector"), Actual, FVector(150.0, 0.0, 0.0));
TestEqual(TEXT("X component"), Actual.X, 150.0, 0.01);   // double literals — OK
// TestEqual(TEXT("X component"), Actual.X, 150.0f, 0.01f); // float literals — C2666
...
**GOTCHA — composite masks are free-standing constants (UE 5.5+).** `EAutomationTestFlags` is an `enum class`, so its individual flags keep the scoped form (`EAutomationTestFlags::EditorContext`), but the COMPOSITE MASKS are spelled with an underscore as free-standing constants:
...
The scoped form `EAutomationTestFlags::ApplicationContextMask` does **NOT** compile. Correct usage:

```cpp
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FMyTest, "MyGame.Area.Behaviour",
...
**Key profiling commands**: `Trace.Start cpu,frame`/`Trace.Stop` (writes a `.utrace` for Unreal Insights — the legacy `stat startfile` `.uestats` format is NOT an Insights format and cannot be opened by it), `stat gpu`/`ProfileGPU` (GPU timing), `stat memoryplatform`/`memreport -full` (memory), `-trace=cpu,gpu,frame,memory` (Insights launch args). See `references/profiling-commands.md`.
