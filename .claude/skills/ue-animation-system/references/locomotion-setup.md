...
5. **Layered Blend Per Bone** — blends upper body slot over the state machine
   - Base Pose: State Machine output (lower body + full)
   - Blend Poses 0: UpperBody slot
   - Layer Setup: Bone Name `spine_01`, Blend Depth `0`
6. **Output Pose**
...
5. In the **Layer Setup** array, add entry:
   - Bone Name: `spine_01`
   - Blend Depth: `0` (bones from `spine_01` outward are fully blended;
