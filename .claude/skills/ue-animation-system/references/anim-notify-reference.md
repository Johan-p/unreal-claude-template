...
**Header**: `AnimNotify_PlayNiagaraEffect.h` (Niagara plugin, `NiagaraAnimNotifies` module)

Spawns a `UNiagaraSystem` at a socket location. This is the primary VFX notify for new code.
...
UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "AnimNotify")
uint32 Attached : 1;   // attach to the bone/socket vs spawn at location
...
**Header**: `AnimNotifyState_TimedNiagaraEffect.h` (Niagara plugin, `NiagaraAnimNotifies` module)

Activates a looping `UNiagaraSystem` for the duration of the notify state.
Deactivates (or destroys, via `bDestroyAtEnd`) on end. Primary timed-VFX notify state for new code.

Key properties: `Template` (UNiagaraSystem), `SocketName`, `LocationOffset`, `RotationOffset`, `Scale`, `bApplyRateScaleAsTimeDilation`, `bDestroyAtEnd`.

Use for: weapon trails, aura effects, charged attack indicators.

---

### UAnimNotify_PlayParticleEffect — Legacy (Cascade, deprecated since UE5.0) — do not use in new code
...
### UAnimNotifyState_TimedParticleEffect — Legacy (Cascade, deprecated since UE5.0) — do not use in new code
