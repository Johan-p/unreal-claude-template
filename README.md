...
| **Unreal Engine reference** | 32 `ue-*` skills covering the engine's major systems — C++ foundations, gameplay framework, actors/components, input, UI, audio, materials, Niagara, physics, data assets, save games, AI/navigation, state trees, animation, character movement, async/threading, networking, GAS, editor tools, sequencer, world/streaming, PCG, game features, Mass Entity, testing/debugging, module/build system | Pattern references; they also load automatically when their domain comes up |
| **Animation (authored for this template)** | `ue-motion-matching` (PoseSearch schemas/databases, Choosers, Blend Stack, the selection debugger), `ue-anim-warping-locomotion` (orientation/stride warping, motion warping, distance matching, foot placement — including the nodes the official docs omit), `ue-character-rigging-retargeting` (IK Rig, IK Retargeter, foot-slide fixes, runtime retargeting, modular characters), `ue-physics-animation` (ragdoll enter/exit, hit reactions, Physics Control, Rigid Body/AnimDynamics), `ue-control-rig-authoring` (rig building, FBIK, modular rigs, Python rigging), `ue-animation-performance` (Rewind Debugger, Insights, URO, compression, crowd tools); `ue-animation-system` is a ground-up v2 (threading model, Property Access, inertialization rules, slots/sync groups) | Written from ~25 UE 5.8 doc pages cross-checked against Epic's Game Animation Sample — [docs/game-animation-sample-analysis.md](docs/game-animation-sample-analysis.md) is the shared sample map they reference |
...
- Most `ue-*` reference skills originate from [quodsoler/unreal-engine-skills](https://github.com/quodsoler/unreal-engine-skills). The animation group (`ue-motion-matching`, `ue-anim-warping-locomotion`, `ue-character-rigging-retargeting`, `ue-physics-animation`, `ue-control-rig-authoring`, `ue-animation-performance`, and the v2 of `ue-animation-system`) is authored for this template from the UE 5.8 documentation and Epic's Game Animation Sample.
